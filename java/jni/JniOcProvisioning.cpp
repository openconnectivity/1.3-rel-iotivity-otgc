...
static JniPeerCnVerifyListener *jniPeerCnVerifyListener = nullptr;
...
CAResult_t peerCnVerifyCB(const unsigned char* cn, size_t cnLen)
{
    CAResult_t res;

    if (jniPeerCnVerifyListener)
    {
        res = jniPeerCnVerifyListener->peerCnVerifyCallback(cn, cnLen);
    }
    else
    {
        res = CA_STATUS_FAILED;
        LOGE("PeerCnVerifyListener is null");
    }
    return res;
}
...
JNIEXPORT jobject JNICALL Java_org_iotivity_base_OcProvisioning_discoverSingleDevice
  (JNIEnv *env, jclass clazz , jint timeout, jstring jDeviceId)
{
    OC_UNUSED(clazz);
    LOGI("OcProvisioning_discoverSingleDevice");
    OicUuid_t deviceId;
    ...
    try
    {
        ...
        OCStackResult result = OCSecure::discoverSingleDevice((unsigned short)timeout, &deviceId, device);
        if (OC_STACK_OK != result)
        {
...
JNIEXPORT jobject JNICALL Java_org_iotivity_base_OcProvisioning_discoverSingleDeviceInUnicast
  (JNIEnv *env, jclass clazz, jint jTimeout, jstring jDeviceId, jstring jHost, jint jConnType)
{
    OC_UNUSED(clazz);
    LOGI("OcProvisioning_discoverSingleDeviceInUnicast");
    OicUuid_t deviceId;
    std::shared_ptr<OC::OCSecureResource> device;
    ...
    if (!jHost)
    {
        ThrowOcException(OC_STACK_INVALID_PARAM, "Host Address cannot be null");
        return nullptr;
    }
    ...
    try
    {
        ...
        std::string host = env->GetStringUTFChars(jHost, nullptr);

        OCStackResult result = OCSecure::discoverSingleDeviceInUnicast(
...
(JNIEnv *env, jclass thiz, jbyteArray jcaCertPem, jbyteArray jcaKeyPem)
{
    OC_UNUSED(thiz);
    LOGD("OcProvisioning_provisionOwnCert");
#if defined(__WITH_DTLS__) || defined(__WITH_TLS__)
	jbyte* caCertPemBytes = env->GetByteArrayElements(jcaCertPem, 0);
	OC_UNUSED(caCertPemBytes);
	jsize certArrayLength = env->GetArrayLength(jcaCertPem);
	
	jbyte* caKeyPemBytes = env->GetByteArrayElements(jcaKeyPem, 0);
	OC_UNUSED(caKeyPemBytes);
	jsize keyArrayLength = env->GetArrayLength(jcaKeyPem);
	
    uint16_t credId = USHRT_MAX;
    unsigned char* caCertPem = new unsigned char[certArrayLength];
    unsigned char* caKeyPem = new unsigned char[keyArrayLength];
    try
    {
        env->GetByteArrayRegion (jcaCertPem, 0, certArrayLength, reinterpret_cast<jbyte*>(caCertPem));
        env->GetByteArrayRegion (jcaKeyPem, 0, keyArrayLength, reinterpret_cast<jbyte*>(caKeyPem));
        OCStackResult result = OCSecure::provisionOwnCert((uint8_t*)caCertPem, certArrayLength, (uint8_t*)caKeyPem, keyArrayLength, &credId);
...
{
    OC_UNUSED(clazz);
    LOGI("OcProvisioning_setPeerCnVerifyListener");

    if (!jListener)
    {
        ThrowOcException(OC_STACK_INVALID_CALLBACK, "Listener can't be null");
        return;
    }
    delete jniPeerCnVerifyListener;
    jniPeerCnVerifyListener = new JniPeerCnVerifyListener(env, jListener);

    try
    {
        OCStackResult result = OCSecure::setPeerCNVerifyCallback(peerCnVerifyCB);
        if (OC_STACK_OK != result)
        {
            ThrowOcException(result, "Failed to set Listener");
            return;
        }
    }
    catch (OCException& e)
...
{
    OC_UNUSED(thiz);
    LOGD("OcProvisioning_resetSvrDb");
    try
    {
        OCStackResult result = OCSecure::resetSvrDb();
        if (OC_STACK_OK != result)
        {
            ThrowOcException(result, "OCSecure::resetSvrDb Failed");
            return;
        }
    }
    catch (OCException& e)
