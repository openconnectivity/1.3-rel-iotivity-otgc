 ...
 * @param cborSize is the size of the CBOR.
 * @param secureFlag shows fill or not private key.
 * @param withRtAndIf shows fill or not resource type and interface
 *
 * @return ::OC_STACK_OK if conversion is successful, else ::OC_STACK_ERROR if unsuccessful.
 */
OCStackResult CredToCBORPayloadWithRtAndIf(const OicSecCred_t* cred, uint8_t **cborPayload,
...
// This function converts CBOR format to Cred data.
