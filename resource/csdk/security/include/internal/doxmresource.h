 ...
 * @return ::OC_STACK_OK for Success, otherwise some error value.
 */
 OCStackResult DoxmToCBORPayloadPartialWithRtAndIf(const OicSecDoxm_t *doxm,
 	uint8_t **payload, size_t *size, const bool *propertiesToInclude, const bool withRtAndIf);
 ...
 * @param checkDeviceId true if included deviceuuid property in the comparison.
 *                      false otherwise.
 *
 * @return true if all of the properties are identical, else false.
 */
bool AreDoxmBinPropertyValuesEqual(OicSecDoxm_t* doxm1, OicSecDoxm_t* doxm2, bool checkDeviceId);
