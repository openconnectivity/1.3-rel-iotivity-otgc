...
typedef struct OCPMGetAclResult
{
	OCStackResult      	res;
    OicSecAcl_t         *acl;
} OCPMGetAclResult_t;

typedef struct OCPMGetCredsResult
{
    OCStackResult       res;
    OicSecCreds_t       *creds;
} OCPMGetCredsResult_t;
...
/**
 * Callback function definition of ACL retrieve API
 *
 * @param[OUT] ctx - If user set a context, it will be returned here.
 * @param[OUT] res - OCPMGetAclResult_t, containing one entry for each target device. If an entry's res
 *                   member is OC_STACK_OK, then acl and aclLen are valid; otherwise they should not be used.
 ...
 */
typedef void (*OCGetACLResultCB)(void* ctx, OCPMGetAclResult_t *res, bool hasError);

/**
 * Callback function definition of Cred retrieve API
 *
 * @param[OUT] ctx - If user set a context, it will be returned here.
 * @param[OUT] res - OCPMGetCredResult_t, containing one entry for each target device. If an entry's res
 *                   member is OC_STACK_OK, then cred and credLen are valid; otherwise they should not be used.
 *                   This memory is only valid while the callback is executing; callers must make copies if the data
 *                   needs to be kept longer.
 * @param[OUT] hasError - If all calls succeeded, this will be false. One or more errors, and this will
 *                        be true. Examine the elements of res to discover which failed.
 */
typedef void (*OCGetCredsResultCB)(void* ctx, OCPMGetCredsResult_t *res, bool hasError);
