...
/**
 * API to delete version 1 ACE from device.
 *
 * @param[in] ctx Application context returned in the result callback.
 * @param[in] selectedDeviceInfo Selected target device.
 * @param[in] aceId ID of the ACE to be deleted.
 * @param[in] resultCallback callback provided by API user, callback will be called when provisioning
              request recieves a response from resource server.
 * @return OC_STACK_OK in case of success and other value otherwise.
 */
OCStackResult OC_CALL OCDeleteACE(void *ctx, const OCProvisionDev_t *selectedDeviceInfo, uint16_t aceId,
                              OCProvisionResultCB resultCallback)
{
    ...
    return SRPDeleteACEById(ctx, selectedDeviceInfo, aceId, aclVersion, resultCallback);
}

/**
 * API to delete version 2 ACE from device.
 *
 * @param[in] ctx Application context returned in the result callback.
 * @param[in] selectedDeviceInfo Selected target device.
 * @param[in] aceId ID of the ACE to be deleted.
 ...
 * @return OC_STACK_OK in case of success and other value otherwise.
 */
OCStackResult OC_CALL OCDeleteACE2(void *ctx, const OCProvisionDev_t *selectedDeviceInfo, uint16_t aceId,
                              OCProvisionResultCB resultCallback)
{
    return SRPDeleteACEById(ctx, selectedDeviceInfo, aceId, OIC_SEC_ACL_V2, resultCallback);
}
...
OCStackResult OC_CALL OCGetACLResource(void* ctx, const OCProvisionDev_t *selectedDeviceInfo,
                                       OCGetACLResultCB resultCallback)
{
...
OCStackResult OC_CALL OCDeleteCredentialByCredId(void *ctx, const OCProvisionDev_t *selectedDeviceInfo,
                                                 uint16_t credId, OCProvisionResultCB resultCallback)
{
    return SRPDeleteCredentialByCredId(ctx, selectedDeviceInfo, credId, resultCallback);
}
...
* @return OC_STACK_OK in case of success and other value otherwise.
*/
OCStackResult OC_CALL OCProvisionIdentityCertificate(void *ctx,
    const OCProvisionDev_t *pDev,
    OCProvisionResultCB resultCallback)
{
    return SRPProvisionIdentityOrRoleCertificate(ctx, pDev, NULL, NULL, resultCallback);
}
...
* @return OC_STACK_OK in case of success and other value otherwise.
*/
OCStackResult OC_CALL OCProvisionRoleCertificate(void *ctx,
    const OCProvisionDev_t *pDev,
    const char* role, const char* authority,
    OCProvisionResultCB resultCallback)
{
    return SRPProvisionIdentityOrRoleCertificate(ctx, pDev, role, authority, resultCallback);
}
#endif
 ...
 * @return OC_STACK_OK in case of success and other value otherwise.
 */
OCStackResult OC_CALL OCSelectOTMethod(void *ctx, const OCProvisionDev_t *targetDeviceInfo,
                                        const OicSecOxm_t oxmSelValue, OCProvisionResultCB resultCallback)
{
    return OTMSetOTMethod(ctx, targetDeviceInfo, oxmSelValue, resultCallback);
}
 ...
 * @return  OC_STACK_OK in case of success and other value otherwise.
 */
OCStackResult OC_CALL OCProvisionOwnCert(const uint8_t *caCertPem, size_t caCertPemSize, const uint8_t *caKeyPem, size_t caKeyPemSize, uint16_t *credId)
{
	return SRPProvisionOwnCert(caCertPem, caCertPemSize, caKeyPem, caKeyPemSize, credId);
}
...
/**
 * Function to save a manufacturer certificate chain into Cred of SVR.
 *
 * @param[in] cert Certificate chain to be saved in Cred of SVR, PEM encoded, null terminated
 * @param[in] key key corresponding to the certificate, PEM encoded, null terminated
 * @param[out] credId CredId of saved certificate chain in Cred of SVR.
 * @return  OC_STACK_OK in case of success and other value otherwise.
 */
OCStackResult OC_CALL OCSaveOwnMfgCertChain(const char* cert, const char* key, uint16_t *credId)
{
    OicSecKey_t ownCert = { 0 };
    ownCert.data = (uint8_t*) cert;
    ownCert.len = strlen(cert) + 1;
    ...
    return SRPSaveOwnMfgCertChain(&ownCert, &ownKey, credId);
}
