 ...
 */
static OCStackApplicationResult OTMUpdateSecurityResourceCB(void *ctx, OCDoHandle handle,
        OCClientResponse *clientResponse)
{
    ...
    // otmCtx = (OTMContext_t *)(((Data_t *)ctx)->ctx);
    otmCtx = (OTMContext_t *)(ctx);

    VERIFY_NOT_NULL(TAG, otmCtx, ERROR);
    VERIFY_NOT_NULL(TAG, otmCtx->ctxResultCallback, ERROR);
    VERIFY_NOT_NULL(TAG, otmCtx->ctxResultArray, ERROR);

    if (clientResponse)
    {
        memcpy(otmCtx->ctxResultArray[0].deviceId.id,
               otmCtx->selectedDeviceInfo->doxm->deviceID.id,
               sizeof(OicUuid_t));
        otmCtx->ctxResultArray[0].res = clientResponse->result;
    ...
    else
    {
        ...
        otmCtx->ctxResultArray[0].res = OC_STACK_ERROR;
        otmCtx->ctxHasError = true;
    }
...
exit:
    if (otmCtx)
    {
        PMDeleteDeviceList(otmCtx->selectedDeviceInfo);
 ...
 */
static OCStackResult OTMSendPostDoxm(void *ctx,
                                     ...
                                     OicSecDoxm_t *doxm)
{
    ...
    OCStackResult postOtmRes =  OC_STACK_ERROR;

    //Create the OTM Context to handle the response message
    otmCtx = (OTMContext_t *)OICCalloc(1, sizeof(OTMContext_t));
    VERIFY_NOT_NULL(TAG, otmCtx, ERROR);

    localTargetDeviceInfo = PMCloneOCProvisionDevList(targetDeviceInfo);
    VERIFY_NOT_NULL(TAG, localTargetDeviceInfo, ERROR);

    otmCtx->selectedDeviceInfo = localTargetDeviceInfo;
    otmCtx->ctxResultCallback = resultCallback;
    otmCtx->ctxResultArraySize = 1;
    otmCtx->ctxHasError = false;
    otmCtx->userCtx = ctx;
    otmCtx->ctxResultArray = (OCProvisionResult_t *)OICCalloc(1, sizeof(OCProvisionResult_t));
    VERIFY_NOT_NULL(TAG, otmCtx->ctxResultArray, ERROR);
	...
	postOtmRes = DoxmToCBORPayloadPartialWithRtAndIf(doxm, &secPayload->securityData,
										  &secPayload->payloadSize, propertiesToInclude, false);
	VERIFY_SUCCESS(TAG, (OC_STACK_OK == postOtmRes), ERROR);
	...
	cbData.cb = &OTMUpdateSecurityResourceCB;
	cbData.context = otmCtx;
	cbData.cd = NULL;
	
	postOtmRes = OCDoRequest(NULL, OC_REST_POST, query,
 ...
 * @param[in] oxmSelValue Method of ownership transfer (ref. oic.sec.doxmtype)
 * @param[in] resultCallback callback provided by API user, callback will be called when
 *            POST 'oxmsel' request recieves a response from resource server.
 * @return OC_STACK_OK in case of success and other value otherwise.
 */
OCStackResult OTMSetOTMethod(void *ctx, const OCProvisionDev_t *targetDeviceInfo,
                                 const OicSecOxm_t oxmSelValue, OCProvisionResultCB resultCallback)
{
    OCStackResult postRes = OC_STACK_INVALID_CALLBACK;
    OicSecDoxm_t *doxm = NULL;
    uint8_t *doxmPayload = NULL;
    size_t doxmPayloadLen = 0;

    OIC_LOG(DEBUG, TAG, "IN OTMSetOTMethod");

    VERIFY_NOT_NULL(TAG, resultCallback, ERROR);
    postRes = OC_STACK_INVALID_PARAM;
    ...
    //Duplicate doxm resource to update the 'oxmsel' property
    postRes = DoxmToCBORPayload(targetDeviceInfo->doxm, &doxmPayload, &doxmPayloadLen);
    VERIFY_SUCCESS(TAG, (OC_STACK_OK == postRes), ERROR);

    postRes = CBORPayloadToDoxm(doxmPayload, doxmPayloadLen, &doxm);
    VERIFY_SUCCESS(TAG, (OC_STACK_OK == postRes), ERROR);
    VERIFY_NOT_NULL(TAG, doxm, ERROR);

    doxm->oxmSel = oxmSelValue;

    //Send POST request for update doxm
    postRes = OTMSendPostDoxm(ctx, targetDeviceInfo, resultCallback, doxm);
    VERIFY_SUCCESS(TAG, (OC_STACK_OK == postRes), ERROR);

    OIC_LOG(DEBUG, TAG, "OUT OTMSetOTMethod");
        ...
        size_t numberOfMethods, OicSecOxm_t *selectedMethod, OwnerType_t ownerType)
{
    ...
    OicSecOxm_t *tmp = selectedMethod;
        ...
        case SUPER_OWNER:
        {
            for (size_t i = 0; i < numberOfMethods; i++)
            {
                ...
                if (*selectedMethod == supportedMethods[i] && 
                	ALLOWED_OXM == g_OxmAllowStatus[selectedOxmIdx])
            	{
                    isOxmSelected = true;
                    isChanged = false;
                    break;
...
#ifdef MULTIPLE_OWNER
                else if (ALLOWED_OXM == g_OxmAllowStatus[selectedOxmIdx] &&
                   OXM_IDX_PRECONFIG_PIN != selectedOxmIdx && !isOxmSelected)
#else
                else if (ALLOWED_OXM == g_OxmAllowStatus[selectedOxmIdx] && !isOxmSelected)
#endif //MULTIPLE_OWNER
                {
                    *tmp  = supportedMethods[i];
                    isOxmSelected = true;
                    isChanged = true;
                }
            }
    ...
    OIC_LOG_V(DEBUG, TAG, "Selected OTM Method: %d", *selectedMethod);
...
static OCStackApplicationResult GetCredentialsSupportedDoxmHandler(void *ctx, OCDoHandle UNUSED,
                                                    OCClientResponse *clientResponse)
{
    OIC_LOG_V(DEBUG, TAG, "IN %s", __func__);
    ...
    else
    {
        ...
        bool equal = AreDoxmBinPropertyValuesEqual(doxm, deviceInfo->doxm, false);
        
        if (equal
                && 0 != memcmp(&doxm->deviceID, &deviceInfo->doxm->deviceID, sizeof(doxm->deviceID)))
        {
            OIC_LOG(INFO, TAG, "DeviceID has changed");

            res = PDMUpdateUuid(&deviceInfo->doxm->deviceID, &doxm->deviceID);
            if (OC_STACK_OK != res)
            {
                OIC_LOG_V(ERROR, TAG, "Internal error in PDMUpdateUuid : %d", res);
                DeleteDoxmBinData(doxm);
                SetResult(otmCtx, OC_STACK_ERROR);
                return OC_STACK_DELETE_TRANSACTION;
            }
            ...
            for(size_t i = 0; i < otmCtx->ctxResultArraySize; i++)
            {
                if(memcmp(deviceInfo->doxm->deviceID.id,
                          otmCtx->ctxResultArray[i].deviceId.id, UUID_LENGTH) == 0)
                {
            ...
            memcpy(&deviceInfo->doxm->deviceID.id, &doxm->deviceID.id, UUID_LENGTH);            
        }
        ...
        res = SaveOwnerPSK(deviceInfo);
        if(OC_STACK_OK != res)
        {
            OIC_LOG(ERROR, TAG, "GetCredentialsSupportedDoxmHandler:Failed to owner PSK generation");
            SetResult(otmCtx, res);
            return OC_STACK_DELETE_TRANSACTION;
        }
        ...
        if(OC_STACK_OK != res)
        {
            OIC_LOG(ERROR, TAG, "Failed to send POST request for owner credentials");
            SetResult(otmCtx, res);
        }
    }
...
static OCStackResult GetCredentialsSupportedDoxm(OTMContext_t* otmCtx)
{
    OIC_LOG_V(DEBUG, TAG, "IN %s", __func__);
    ...
    OCCallbackData cbData;
    cbData.cb = &GetCredentialsSupportedDoxmHandler;
                                ...
                                OCClientResponse *clientResponse)
{
    ...
    if(OC_STACK_RESOURCE_CHANGED == clientResponse->result)
    {
        if(otmCtx && otmCtx->selectedDeviceInfo)
        {
            ...
            //POST owner credential to new device according to security spec B.
            //res = PostOwnerCredential(otmCtx);
            res = GetCredentialsSupportedDoxm(otmCtx);
            if(OC_STACK_OK != res)
            {
                OIC_LOG(ERROR, TAG,
                        "OwnerUuidUpdateHandler:Failed to send Get request for credential supported");
                SetResult(otmCtx, res);
                return OC_STACK_DELETE_TRANSACTION;
            }
        }
 ...
 *          and  OC_STACK_KEEP_TRANSACTION to keep it.
 */
static OCStackApplicationResult RownerUuidCredentialHandler(void *ctx, OCDoHandle UNUSED,
                                OCClientResponse *clientResponse)
{
    OIC_LOG(DEBUG, TAG, "IN RownerUuidCredentialHandler");
    ...
    if(OC_STACK_RESOURCE_CHANGED == res)
    {
        ...
        {
            OIC_LOG_V(DEBUG, TAG, "%s posting /doxm.rowneruuid.", __func__);
        ...
        {
            OIC_LOG_V(ERROR, TAG, "RownerUuidCredentialHandler : Unexpected result %d", res);
            SetResult(otmCtx, res);
        }
    }

        OIC_LOG(DEBUG, TAG, "OUT RownerUuidCredentialHandler");

    exit:
        return  OC_STACK_DELETE_TRANSACTION;
}

static OCStackResult PostRownerUuidCredential(OTMContext_t* otmCtx)
{
    OIC_LOG(DEBUG, TAG, "IN PostRownerUuidCredential");
    ...
    {
        OIC_LOG(ERROR, TAG, "PostRownerUuidCredential : Failed to generate query");
        return OC_STACK_ERROR;
    }
    OIC_LOG_V(DEBUG, TAG, "Query=%s", query);
    OCSecurityPayload* secPayload = (OCSecurityPayload*)OICCalloc(1, sizeof(OCSecurityPayload));
    ...
    if(OC_STACK_OK == GetDoxmDeviceID(&credSubjectId))
    {
        ...
        if (OC_STACK_OK != CredToCBORPayloadWithRownerWithRtAndIf(&newCredential, &credSubjectId, &secPayload->securityData,
                                        &secPayload->payloadSize, secureFlag, false))
		{
            OICFree(secPayload);
        ...
        OCCallbackData cbData;
        cbData.cb = &RownerUuidCredentialHandler;
        cbData.context = (void *)otmCtx;
        cbData.cd = NULL;
                                ...
                                OCClientResponse *clientResponse)
{
    ...
    if(OC_STACK_RESOURCE_CHANGED == res)
    {
        if(NULL != selectedDeviceInfo)
        {
            ...
            //res = PostOwnershipInformation(otmCtx);
            res = PostRownerUuidCredential(otmCtx);
            if(OC_STACK_OK != res)
            {
...
static OCStackApplicationResult OwnedHandler(void *ctx, OCDoHandle UNUSED,
                                OCClientResponse *clientResponse)
{
    VERIFY_NOT_NULL(TAG, clientResponse, WARNING);
    VERIFY_NOT_NULL(TAG, ctx, WARNING);

    OIC_LOG(DEBUG, TAG, "IN OwnedHandler");
    (void)UNUSED;
    OTMContext_t* otmCtx = (OTMContext_t*)ctx;
    otmCtx->ocDoHandle = NULL;
    ...
    {
        res = clientResponse->result;
        OIC_LOG_V(ERROR, TAG, "OwnedHandler : Unexpected result %d", res);
        SetResult(otmCtx, res);
    }

    OIC_LOG(DEBUG, TAG, "OUT OwnedHandler");
                                                       ...
                                                       OCClientResponse *clientResponse)
{
    ...
    if(OC_STACK_RESOURCE_CHANGED == clientResponse->result)
    {
        if(otmCtx && otmCtx->selectedDeviceInfo)
        {
            OIC_LOG(INFO, TAG, "Device state is in Ready for Provisioning.");
                                                    ...
                                                    OCClientResponse *clientResponse)
{
    ...
    else
    {
        ...
        bool equal = AreDoxmBinPropertyValuesEqual(doxm, deviceInfo->doxm, true);
        DeleteDoxmBinData(doxm);
        if (!equal)
        {
            SetResult(otmCtx, OC_STACK_ERROR);
...
static OCStackResult PostOwnerCredential(OTMContext_t* otmCtx)
{
    ...
    if(OC_STACK_OK == GetDoxmDeviceID(&credSubjectId))
    {
        ...
        int secureFlag = 0;
        //Send owner credential to new device : POST /oic/sec/cred [ owner credential ]
        if (OC_STACK_OK != CredToCBORPayloadWithRtAndIf(&newCredential, &secPayload->securityData,
                                        &secPayload->payloadSize, secureFlag, false))
        {
            OICFree(secPayload);
            OIC_LOG(ERROR, TAG, "Error while converting bin to cbor.");
...
static OicSecAcl_t* GenerateOwnerAcl(const OicUuid_t* owner)
{
    ...
    wildcardRsrc->wildcard = ALL_RESOURCES;
...
static OCStackResult PostOwnerAcl(OTMContext_t* otmCtx, OicSecAclVersion_t aclVer)
{
    ...
    res = AclToCBORPayloadWithRtAndIf(ownerAcl, aclVer, &secPayload->securityData, &secPayload->payloadSize, false);
    if (OC_STACK_OK != res)
    {
        OICFree(secPayload);
        OIC_LOG(ERROR, TAG, "Error while converting bin to cbor.");
...
static OCStackResult PostOwnershipInformation(OTMContext_t* otmCtx)
{
    ...
    OCStackResult res = DoxmToCBORPayloadPartialWithRtAndIf(otmCtx->selectedDeviceInfo->doxm,
            &secPayload->securityData, &secPayload->payloadSize,
...
static OCStackResult PostOwned(OTMContext_t* otmCtx)
{
    OIC_LOG_V(DEBUG, TAG, "IN %s", __func__);
...
static OCStackResult PostUpdateOperationMode(OTMContext_t* otmCtx)
{
    ...
    OCStackResult res = PstatToCBORPayloadPartialWithRtAndIf(deviceInfo->pstat, &secPayload->securityData,
...
OCStackResult PostProvisioningStatus(OTMContext_t* otmCtx)
{
    ...
    if (OC_STACK_OK != PstatToCBORPayloadPartialWithRtAndIf(otmCtx->selectedDeviceInfo->pstat,
            &secPayload->securityData, &secPayload->payloadSize, propertiesToInclude, false, false))
    {
        OCPayloadDestroy((OCPayload *)secPayload);
                                                       ...
                                                       OCClientResponse *clientResponse)
{
    ...
    if(OC_STACK_RESOURCE_CHANGED == clientResponse->result)
    {
    	if(otmCtx && otmCtx->selectedDeviceInfo)
        {
		    res = PostOwned(otmCtx);
		    if(OC_STACK_OK != res)
		    {
		        OIC_LOG(ERROR, TAG, "Failed to set owned = true");
		        SetResult(otmCtx, res);
		    }
        ...
        {
            res = clientResponse->result;
            OIC_LOG_V(ERROR, TAG, "RownerUuidHandler : Unexpected result %d", res);
            SetResult(otmCtx, res);
        }

        OIC_LOG(DEBUG, TAG, "OUT RownerUuidHandler");
    }
exit:
    return OC_STACK_DELETE_TRANSACTION;
}
