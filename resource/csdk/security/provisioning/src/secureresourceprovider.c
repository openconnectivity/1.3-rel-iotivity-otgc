...
/**
 * Structure to carry Creds provision API data to callback.
 */
typedef struct CredsData
{
    void *ctx;
    const OCProvisionDev_t *deviceInfo;
...
typedef struct GetAclData GetAclData_t;
struct GetAclData {
    void *ctx;
    OCGetACLResultCB resultCallback;            /**< Pointer to result callback.**/
    OCPMGetAclResult_t *res;                  	/**< Result.**/
};

typedef struct GetCredsData GetCredsData_t;
struct GetCredsData {
    void *ctx;
    OCGetCredsResultCB resultCallback;          /**< Pointer to result callback.**/
    OCPMGetCredsResult_t *res;                  /**< Result.**/
};
        ...
        OCClientResponse *clientResponse)
{
    ...
    switch (dataType)
    {
        ...
        case CREDS_TYPE:
        {
            CredsData_t *credsData = (CredsData_t *) ((Data_t *) ctx)->ctx;
            resultCallback = credsData->resultCallback;
            targetDev = credsData->deviceInfo;
            resArr = credsData->resArr;
            numOfResults = &(credsData->numOfResults);
                            ...
                            OCClientResponseHandler resultCallback)
{
    ...
    if (OC_STACK_OK != PstatToCBORPayloadPartialWithRtAndIf(pstat, &(secPayload->securityData),
            &(secPayload->payloadSize), propertiesToInclude, false, false))
    {
        OCPayloadDestroy((OCPayload *) secPayload);
        ...
        OCClientResponse *clientResponse)
{
    ...
    {
        OIC_LOG_V(ERROR, TAG, "Response result: %d", clientResponse->result);
    }
...
/**
 * Restores pstat after delete.
 */
static OCStackApplicationResult DeleteCB(void *ctx, OCDoHandle handle,
        OCClientResponse *clientResponse)
{
    OIC_LOG_V(DEBUG, TAG, "IN %s", __func__);
    VERIFY_NOT_NULL_RETURN(TAG, ctx, ERROR, OC_STACK_DELETE_TRANSACTION);
    OC_UNUSED(handle);
    if (clientResponse && OC_STACK_RESOURCE_DELETED != clientResponse->result)
    {
        OIC_LOG_V(ERROR, TAG, "Response result: %d", clientResponse->result);
    }
    if (OC_STACK_OK != SetDOS(ctx, DOS_RFNOP, SetReadyForNormalOperationCB))
...
/**
 * function to provision an identity certificate.
 *
 * @param[in] caCertPem CA certificate.
 * @param[in] caKeyPem CA private key.
 * @param[out] credId CredId of own certificate provisioned.
 * @return  OC_STACK_OK in case of success and other value otherwise.
 */
OCStackResult SRPProvisionOwnCert(const uint8_t *caCertPem, size_t caCertPemSize, const uint8_t *caKeyPem, size_t caKeyPemSize, uint16_t *credId)
{
	time_t nowTimeT;
    struct tm *now;
    ...
    OIC_LOG(DEBUG, TAG, "IN SRPProvisionOwnCert");
    VERIFY_NOT_NULL_RETURN(TAG, caCertPem, ERROR,  OC_STACK_INVALID_PARAM);
    VERIFY_NOT_NULL_RETURN(TAG, caKeyPem, ERROR,  OC_STACK_INVALID_PARAM);
    VERIFY_NOT_NULL_RETURN(TAG, credId, ERROR,  OC_STACK_INVALID_PARAM);
    ...
    uint8_t *certPem = (uint8_t *)OICCalloc(1, caCertPemSize + 1);
    memcpy(certPem, caCertPem, caCertPemSize);
    uint8_t *keyPem = (uint8_t *)OICCalloc(1, caKeyPemSize + 1);
    memcpy(keyPem, caKeyPem, caKeyPemSize);
    ...
    // Add credential as mfgcert
    uint16_t credIdMfgCert = 0;
    res = OCSaveOwnMfgCertChain(idCert, idKey, &credIdMfgCert);
    if (res != OC_STACK_OK)
    {
    	OIC_LOG_V(ERROR, TAG, "Failed to add credential with usage mfgcert, error: %d", res);
    	goto exit;
    }

    *credId = 0;
    res = OCSaveOwnCertChain(idCert, idKey, credId);
    if (res != OC_STACK_OK)
    {
...
OCStackResult SRPSaveOwnMfgCertChain(OicSecKey_t * cert, OicSecKey_t * key, uint16_t *credId)
{
    return saveCertChain(cert, key, credId, MF_PRIMARY_CERT);
}
/**
        ...
        OCClientResponse *clientResponse)
{
    ...
    VERIFY_SUCCESS(TAG, OC_STACK_OK == CredToCBORPayloadToAddCertificate(cred, &secPayload->securityData,
...
/**
 * Restores pstat after provisioning credential.
 */
 ...
 */
static OCStackResult createIdentityCertFromCSR(const char* caKeyPem, const char* caCertPem, char* csr, char** deviceCert)
{
    char* publicKey = NULL;
    char* serial = NULL;
    size_t serialLen;
    ...
    char* TEST_CERT_NOT_BEFORE = "20170101000000"; // Not before field for certificates, in format YYYYMMDDhhmmss
	char* TEST_CERT_NOT_AFTER = "20270101000000";  // + ten years
...
/*
 * Create a role certificate for a device, based on the information in its CSR.
 * Assumes the csr has already been validated wtih OCVerifyCSRSignature.
 */
static OCStackResult createRoleCertFromCSR(const char* role, const char* authority, const char* caKeyPem, const char* caCertPem, char* csr, char** deviceCert)
{
    char* publicKey = NULL;
    char* serial = NULL;
    size_t serialLen;
    ...
    if(res != OC_STACK_OK)
    {
        OIC_LOG_V(ERROR, TAG, "OCGenerateRoleCertificate failed, error: %d", res);
        goto exit;
    }
...
void getCsrForCertProvCB(void *ctx, size_t nOfRes, OCPMGetCsrResult_t* arr, bool hasError)
{
	OIC_LOG_V(DEBUG, TAG, "IN %s", __func__);
    ...
    if (!hasError)
    {
        ...
        {
            res = OCConvertDerCSRToPem((char*)arr[0].csr, arr[0].csrLen, &csr);
            if (res != OC_STACK_OK)
            {
        ...
        {
            csr = (char*)OICCalloc(1, arr[0].csrLen);
            if (csr == NULL)
            {
            ...
            memcpy(csr, arr[0].csr, arr[0].csrLen);
        ...
        //OIC_LOG(DEBUG, TAG, "CSR data: ");
        //OIC_LOG_BUFFER(DEBUG, TAG, csr, strlen(csr));
        //OIC_LOG_V(DEBUG, TAG, "CSR size: %d", strlen(csr));
		...
		{
			res = createIdentityCertFromCSR(caKeyPem, caCertPem, csr, &deviceCert);
		}
		else
		{
		...
		res = SRPProvisionCertificate((void *)ctx, pDev, deviceCert, provisionCredCB);
		if (OC_STACK_OK != res)
		{
...
/**
 * Callback for ACE removal.
 */
static OCStackApplicationResult DeleteACEByIdCB(void *ctx, OCDoHandle UNUSED,
        OCClientResponse *clientResponse);

OCStackResult SRPDeleteACEById(void *ctx, const OCProvisionDev_t *selectedDeviceInfo,
                                        uint16_t aceId, OicSecAclVersion_t aclVersion, OCProvisionResultCB resultCallback)
{
    VERIFY_NOT_NULL_RETURN(TAG, selectedDeviceInfo, ERROR, OC_STACK_INVALID_PARAM);
    ...
    if (SetDOS(data, DOS_RFPRO, DeleteACEByIdCB) != OC_STACK_OK)
    {
        OICFree(ace);
        OICFree(acl);
        FreeData(data);
                                                  ...
                                                  OCClientResponse *clientResponse)
{
    ...
    OC_UNUSED(UNUSED);
    VERIFY_NOT_NULL_RETURN(TAG, ctx, ERROR, OC_STACK_DELETE_TRANSACTION);
    GetCredsData_t *getCredsData = (GetCredsData_t*)ctx;
    OCGetCredsResultCB resultCallback = getCredsData->resultCallback;

    if (clientResponse)
    {
        getCredsData->res->res = clientResponse->result;
        if(OC_STACK_OK == clientResponse->result)
        {
            ...
            OicSecCreds_t* creds = (OicSecCreds_t*)OICCalloc(1, sizeof(OicSecCreds_t));
            if (OC_STACK_OK != CBORPayloadToCred(payload, size, &creds->creds, &creds->rownerID))
            {
                OIC_LOG(ERROR, TAG, "Cred payload is invalid");
...
OCStackResult SRPGetCredResource(void *ctx, const OCProvisionDev_t *selectedDeviceInfo,
        OCGetCredsResultCB resultCallback)
{
    VERIFY_NOT_NULL_RETURN(TAG, selectedDeviceInfo, ERROR,  OC_STACK_INVALID_PARAM);
    VERIFY_NOT_NULL_RETURN(TAG, resultCallback, ERROR,  OC_STACK_INVALID_CALLBACK);
    ...
    GetCredsData_t* getCredsData = (GetCredsData_t*)OICCalloc(1, sizeof(GetCredsData_t));
    if (NULL == getCredsData)
    {
        OIC_LOG(ERROR, TAG, "Unable to allocate memory");
        return OC_STACK_NO_MEMORY;
    }
    getCredsData->resultCallback = resultCallback;
    getCredsData->ctx = ctx;
    getCredsData->res = (OCPMGetCredsResult_t*)OICCalloc(1, sizeof(OCPMGetCredsResult_t));
    if (NULL == getCredsData->res)
    {
        OICFree(getCredsData);
        OIC_LOG(ERROR, TAG, "Unable to allocate memory");
        return OC_STACK_NO_MEMORY;
    }
    cbData.context = (void *)getCredsData;
    cbData.cd = NULL;
                                                  ...
                                                  OCClientResponse *clientResponse)
{
    ...
    OC_UNUSED(UNUSED);
    VERIFY_NOT_NULL_RETURN(TAG, ctx, ERROR, OC_STACK_DELETE_TRANSACTION);
    GetAclData_t *getAclData = (GetAclData_t*)ctx;
    OCGetACLResultCB resultCallback = getAclData->resultCallback;
    
    if (clientResponse)
    {
    	getAclData->res->res = clientResponse->result;
        if(OC_STACK_OK == clientResponse->result)
        {
            uint8_t *payload = ((OCSecurityPayload*)clientResponse->payload)->securityData;
            size_t size = ((OCSecurityPayload*)clientResponse->payload)->payloadSize;
...
OCStackResult SRPGetACLResource(void *ctx, const OCProvisionDev_t *selectedDeviceInfo,
        OicSecAclVersion_t aclVersion, OCGetACLResultCB resultCallback)
{
    VERIFY_NOT_NULL_RETURN(TAG, selectedDeviceInfo, ERROR,  OC_STACK_INVALID_PARAM);
    VERIFY_NOT_NULL_RETURN(TAG, resultCallback, ERROR,  OC_STACK_INVALID_CALLBACK);
    ...
    GetAclData_t* getAclData = (GetAclData_t*)OICCalloc(1, sizeof(GetAclData_t));
    if (NULL == getAclData)
    {
        OIC_LOG(ERROR, TAG, "Unable to allocate memory");
        return OC_STACK_NO_MEMORY;
    }
    getAclData->resultCallback = resultCallback;
    getAclData->ctx = ctx;
    getAclData->res = (OCPMGetAclResult_t*)OICCalloc(1, sizeof(OCPMGetAclResult_t));
    if (NULL == getAclData->res)
    {
        OICFree(getAclData);
        OIC_LOG(ERROR, TAG, "Unable to allocate memory");
        return OC_STACK_NO_MEMORY;
    }
    cbData.context = (void *)getAclData;
    OCMethod method = OC_REST_GET;
    OCDoHandle handle = NULL;
...
/**
 * Callback for Credential removal.
 */
static OCStackApplicationResult DeleteCredentialByIdCB(void *ctx, OCDoHandle UNUSED,
        OCClientResponse *clientResponse);

OCStackResult SRPDeleteCredentialByCredId(void *ctx, const OCProvisionDev_t *selectedDeviceInfo,
                                           uint16_t credId, OCProvisionResultCB resultCallback)
{
    VERIFY_NOT_NULL_RETURN(TAG, selectedDeviceInfo, ERROR, OC_STACK_INVALID_PARAM);
    VERIFY_NOT_NULL_RETURN(TAG, resultCallback, ERROR, OC_STACK_INVALID_PARAM);
    OIC_LOG_V(INFO, TAG, "IN %s", __func__);

    CredsData_t *credsData = (CredsData_t *) OICCalloc(1, sizeof(CredsData_t));
    if (NULL == credsData)
    {
        OIC_LOG(ERROR, TAG, "Memory allocation problem");
        OIC_LOG_V(ERROR, TAG, "OUT %s", __func__);
    ...
    credsData->deviceInfo = selectedDeviceInfo;
    credsData->resultCallback = resultCallback;
    credsData->ctx = ctx;
    credsData->numOfResults = 0;
    
    credsData->resArr = (OCProvisionResult_t *) OICCalloc(1, sizeof(OCProvisionResult_t));
    if (NULL == credsData->resArr)
    {
        OICFree(credsData);
        OIC_LOG(ERROR, TAG, "Unable to allocate memory");
        OIC_LOG_V(ERROR, TAG, "OUT %s", __func__);
    ...
    OicSecCreds_t *credentials = (OicSecCreds_t *) OICCalloc(1, sizeof(OicSecCreds_t));
    if (NULL == credentials)
    {
        OICFree(credsData->resArr);
        OICFree(credsData);
    ...
    LL_APPEND(credentials->creds, cred);
    cred->credId = credId;
    credsData->creds = credentials;
    ...
    data->type = CREDS_TYPE;
    data->ctx = credsData;
    
    if (SetDOS(data, DOS_RFPRO, DeleteCredentialByIdCB) != OC_STACK_OK)
    {
        OICFree(cred);
        OICFree(credentials);
        FreeData(data);
        ...
        OCClientResponse *clientResponse)
{
    ...
    if (OC_STACK_RESOURCE_CHANGED == clientResponse->result)
    {
        ...
        secPayload->base.type = PAYLOAD_TYPE_SECURITY;
        if (OC_STACK_OK != AclToCBORPayloadWithRtAndIf(aclData->acl, aclData->aclVersion, &secPayload->securityData,
                                            &secPayload->payloadSize, false))
        {
            OCPayloadDestroy((OCPayload *)secPayload);
    ...
    if (OC_STACK_RESOURCE_CHANGED == clientResponse->result)
    {
        ...
        switch (aclData->aclVersion)
        {
            case OIC_SEC_ACL_V1:
                OIC_LOG(WARNING, TAG,
                        "%s using ACL v1... this should only be used for provisioning OIC 1.1 and earlier Servers.");
                snprintf(uri, MAX_URI_LENGTH, "%s?aceid=%d", OIC_RSRC_ACL_URI, aclData->acl->aces->aceid);
                break;
            case OIC_SEC_ACL_V2:
                snprintf(uri, MAX_URI_LENGTH, "%s?aceid=%d", OIC_RSRC_ACL2_URI, aclData->acl->aces->aceid);
                break;
            default:
                return OC_STACK_INVALID_PARAM;
        }
        ...
        OCMethod method = OC_REST_DELETE;
        OCDoHandle handle = NULL;
        OIC_LOG(DEBUG, TAG, "Sending ACE delete request to resource server");
        OCStackResult ret = OCDoResource(&handle, method, query,
                                         &aclData->deviceInfo->endpoint, NULL,
                                         aclData->deviceInfo->connType, OC_HIGH_QOS, &cbData, NULL, 0);
        if (ret != OC_STACK_OK)
        {
            OIC_LOG_V(ERROR, TAG, "OUT %s", __func__);
        ...
        OCClientResponse *clientResponse)
{
    ...
    if (OC_STACK_RESOURCE_CHANGED == clientResponse->result)
    {
        ...
        char uri[MAX_URI_LENGTH] = { 0 };
        snprintf(uri, MAX_URI_LENGTH, "%s?credid=%d", OIC_RSRC_CRED_URI, credsData->creds->creds->credId);

        char query[MAX_URI_LENGTH + MAX_QUERY_LENGTH] = {0};
        if (!PMGenerateQuery(true,
                             credsData->deviceInfo->endpoint.addr,
                             credsData->deviceInfo->securePort,
                             credsData->deviceInfo->connType,
                             query, sizeof(query), uri))
        {
            OIC_LOG(ERROR, TAG, "DeviceDiscoveryHandler : Failed to generate query");
            return OC_STACK_ERROR;
        ...
        OCMethod method = OC_REST_DELETE;
        OCDoHandle handle = NULL;
        OIC_LOG(DEBUG, TAG, "Sending Credential delete request to resource server");
        OCStackResult ret = OCDoResource(&handle, method, query,
                                         &credsData->deviceInfo->endpoint, NULL,
                                         credsData->deviceInfo->connType, OC_HIGH_QOS, &cbData, NULL, 0);
        if (ret != OC_STACK_OK)
        {
            OIC_LOG_V(ERROR, TAG, "OUT %s", __func__);
            return OC_STACK_ERROR;
