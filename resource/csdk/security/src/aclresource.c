...
OCStackResult AclToCBORPayloadWithRtAndIf(const OicSecAcl_t *secAcl,
    OicSecAclVersion_t aclVersion, uint8_t **payload, size_t *size, const bool withRtAndIf)
{
    ...
    *payload = NULL;
	...
	size_t aclMapSize = ACL_MAP_SIZE;
	if (!withRtAndIf)
	{
		aclMapSize = aclMapSize - 2;
	}
	
    // Create ACL Map which contains aclist or aclist2, rowneruuid, rt and if
    cborEncoderResult = cbor_encoder_create_map(&encoder, &aclMap, aclMapSize);
...
{
    return AclToCBORPayloadWithRtAndIf(secAcl, aclVersion, payload, size, true);
}
    ...
    OicSecAclVersion_t *versionCheck)
{
    ...
    while (cbor_value_is_valid(&aclMap))
    {
        ...
        if(tagName)
        {
            ...
            else if ((IsNilUuid(&acl->rownerID)) && (NULL != gAcl))
            {
...
static OCEntityHandlerResult HandleACL2PostRequest(const OCEntityHandlerRequest *ehRequest)
{
    ...
    if (payload)
    {
		...
		{
			if (!UuidCmp(&(newAcl->rownerID), &(gAcl->rownerID)))
			{
				OIC_LOG_V(WARNING, TAG, "%s /rowneruuid of acl2 resource is read-only in RFPRO.", __func__);
				ehRet = OC_EH_NOT_ACCEPTABLE;
				goto exit;
			}
		}
