...
OCStackResult CredToCBORPayloadWithRownerWithRtAndIf(const OicSecCred_t *credS, const OicUuid_t *rownerId, uint8_t **cborPayload,
                                size_t *cborSize, int secureFlag, const bool withRtAndIf)
{
    OIC_LOG_V(DEBUG, TAG, "IN %s:", __func__);
...
OCStackResult CredToCBORPayloadToAddCertificate(const OicSecCred_t *credS, uint8_t **cborPayload,
                                size_t *cborSize, int secureFlag)
{
    ...
    *cborPayload = NULL;
    ...
    size_t credRootMapSize = CRED_ROOT_MAP_SIZE - 3;
    ...
    {
        CborEncoder credMap;
        size_t mapSize = CRED_MAP_SIZE - 1;
        size_t inLen = 0;
        if (cred->period)
        {
            mapSize++;
...
OCStackResult CredToCBORPayloadWithRowner(const OicSecCred_t *credS, const OicUuid_t *rownerId, uint8_t **cborPayload,
                                size_t *cborSize, int secureFlag)
{
    return CredToCBORPayloadWithRownerWithRtAndIf(credS, rownerId, cborPayload, cborSize, secureFlag, true);
}

OCStackResult CredToCBORPayloadWithRtAndIf(const OicSecCred_t *credS, uint8_t **cborPayload,
                                size_t *cborSize, int secureFlag, const bool withRtAndIf)
{
    return CredToCBORPayloadWithRownerWithRtAndIf(credS, &gRownerId, cborPayload, cborSize, secureFlag, withRtAndIf);
}
...
static OCEntityHandlerResult HandlePostRequest(OCEntityHandlerRequest* ehRequest)
{
    ...
    if (DOS_RFPRO == dos.state)
    {
    	if (rownerId != NULL)
    	{
    		OIC_LOG_V(WARNING, TAG, "%s /rowneruuid of cred resource is read-only in RFPRO.", __func__);
		    ret = OC_EH_NOT_ACCEPTABLE;
		    goto exit;
    	}
    }
...
{
    const char* pemHeader = "-----BEGIN EC PRIVATE KEY-----\n";
    ...
    if (ret != MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL)
    {
        OIC_LOG_V(ERROR, TAG, "Couldn't convert private key into PEM, failed getting required length: %d", ret);
        return ret;
    }
    ...
    *pem = OICCalloc(1, pemLen + 1);
    if (*pem == NULL)
    {
        OIC_LOG(ERROR, TAG, "Failed to allocate memory for PEM private key");
        return ret;
    }
    ...
    if (ret < 0)
    {
        OIC_LOG_V(ERROR, TAG, "Couldn't convert private key into PEM, failed writing PEM: %d", ret);
        OICFreeAndSetToNull((void **) pem);
        return ret;
    }
