...
OCStackResult CBORPayloadToCSR(const uint8_t *cborPayload, size_t size, uint8_t **csr, size_t *csrLen, OicEncodingType_t *encoding)
{
    ...
    while (cbor_value_is_valid(&csrRootMap))
    {
        ...
        if (NULL != tagName)
        {
            ...
            {
            	OIC_LOG(DEBUG, TAG, "Debugging csr byte");
            ...
            else if (strcmp(tagName, OIC_JSON_CSR_NAME) == 0 && cbor_value_is_text_string(&csrRootMap))
            {
            	OIC_LOG(DEBUG, TAG, "Debugging csr text");
            	char* strCsr = NULL;
                cborFindResult = cbor_value_dup_text_string(&csrRootMap, &strCsr, &cborCsrLen, NULL);
                VERIFY_CBOR_SUCCESS_OR_OUT_OF_MEMORY(TAG, cborFindResult, "Failed finding name in CSR Map");
                // Use our own heap allocator and copy the result in so callers can use OICFree.
                *csr = (uint8_t *)OICCalloc(1, cborCsrLen);
                VERIFY_NOT_NULL(TAG, *csr, ERROR);
                memcpy(*csr, strCsr, cborCsrLen);
                free(strCsr);
                strCsr = NULL;
                *csrLen = cborCsrLen;
                cborCsrLen = 0;
            ...
            {
            	OIC_LOG(DEBUG, TAG, "Debugging encoding");
            ...
            {
            	OIC_LOG(DEBUG, TAG, "Debugging unknown tag");
