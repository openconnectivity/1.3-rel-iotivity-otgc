...
OCStackResult OC_CALL OCGetUuidFromCSR(const char* csr, OicUuid_t* uuid)
{
    ...
    const mbedtls_x509_name *name = &csrObj.subject;
    const char *short_name = NULL;
    mbedtls_x509_name *found = NULL;
    int ret2 = 0;
    while (name != NULL && !found)
    {
        ...
        ret2 = mbedtls_oid_get_attr_short_name(&name->oid, &short_name);
        if (ret2 == 0 && 0 == strcmp("CN", short_name))
        {
            found = mbedtls_calloc(1, sizeof(mbedtls_x509_name));
            found->oid = name->oid;
            found->val = name->val;
            found->next = NULL;
        }
    ...
    if (!found)
    {
        OIC_LOG_V(ERROR, TAG, "CN field not found in Subject");
        mbedtls_x509_csr_free(&csrObj);
        return OC_STACK_ERROR;
    }

    char uuidStr[UUID_STRING_SIZE + sizeof(SUBJECT_PREFIX) - 1] = { 0 };   // Both constants count NULL, subtract one
    ret = mbedtls_x509_dn_gets(uuidStr, sizeof(uuidStr), found);
