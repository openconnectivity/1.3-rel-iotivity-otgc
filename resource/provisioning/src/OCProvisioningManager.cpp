...
namespace OC
{
    ...
    /**
     * Prevent multiple registrations of the peer CN verify callback since
     * the underlying IoTivity stack can only handle having one callback
     * set at a time.
     */
    bool g_displayPeerCnVerifyCallbackRegistered = false;

    void callbackWrapperImpl(void* ctx, size_t nOfRes, OCProvisionResult_t *arr, bool hasError)
    {
    ...
    void OCSecureResource::getAclCallbackWrapper(void* ctx, OCPMGetAclResult_t *res, bool hasError)
    {
        //PMGetAclResult_t *result = nullptr;
        GetAclContext* context = static_cast<GetAclContext*>(ctx);

        /*try
        {
            result = new PMGetAclResult_t;
        }
        catch (std::bad_alloc& e)
        {
            OC_UNUSED(e);
        ...
        result = static_cast<PMGetAclResult_t*>(res);*/

        (context->callback)(res, hasError);
        
        delete context;
    }
    
    void OCSecureResource::getCredCallbackWrapper(void* ctx, OCPMGetCredsResult_t *res, bool hasError)
    {
        GetCredContext* context = static_cast<GetCredContext*>(ctx);
        (context->callback)(res, hasError);
	...
	/**
	 * API to provision an identity certificate.
	 *
	 ...
	 */
	OCStackResult OC_CALL OCSecure::provisionOwnCert(uint8_t *caCertPem, size_t caCertPemSize, uint8_t *caKeyPem, size_t caKeyPemSize, uint16_t *credId)
	{
		if (!caCertPem)
        {
            oclog() <<"caCertPem can't be null";
            return OC_STACK_INVALID_PARAM;
        }
        if (!caKeyPem)
        {
            oclog() <<"caKeyPem can not be null";
            return OC_STACK_INVALID_PARAM;
        }
        ...
        {
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCProvisionOwnCert(caCertPem, caCertPemSize, caKeyPem, caKeyPemSize, credId);
        }
        else
        {
            oclog() <<"Mutex not found";
    ...
    OCStackResult OCSecure::setPeerCNVerifyCallback(PeerCNVerifyCallback peerCnVerify)
    {
        if (!peerCnVerify)
        {
            oclog() << "peerCnVerify can't be null";
            return OC_STACK_INVALID_PARAM;
        }
        else if (g_displayPeerCnVerifyCallbackRegistered)
        {
            oclog() << "Callback for peer Common Name verification already registered.";
            return OC_STACK_DUPLICATE_REQUEST;
        }
        ...
        {
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            OCSetPeerCNVerifyCallback(peerCnVerify);
            g_displayPeerCnVerifyCallbackRegistered = true;
        }
        else
        {
            oclog() << "Mutex not found";
...
#endif
	OCStackResult OCSecureResource::getAclResource(AclResultCallBack resultCallback)
    {
        if (!resultCallback)
        {
        ...
        if (cLock)
        {
            GetAclContext* context = new GetAclContext(resultCallback);

            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCGetACL2Resource(static_cast<void*>(context),
                    devPtr, &OCSecureResource::getAclCallbackWrapper);
        }
        else
        {
            oclog() <<"Mutex not found";
    ...
    OCStackResult OCSecureResource::deleteACE(uint16_t aceId,
            ResultCallBack resultCallback)
    {
        if (!resultCallback)
        {
            oclog() << "result callback cannot be null";
            return OC_STACK_INVALID_CALLBACK;
        }
        ...
        if (cLock)
        {
            ...
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCDeleteACE(static_cast<void*>(context),
                    devPtr, aceId, &OCSecureResource::callbackWrapper);
        }
        else
        {
            oclog() << "Mutex not found";
    ...
    OCStackResult OCSecureResource::getCredResource(CredResultCallBack resultCallback)
    {
        ...
        if (cLock)
        {
            GetCredContext *context = new GetCredContext(resultCallback);
            
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCGetCredResource(static_cast<void*>(context),
                    devPtr, &OCSecureResource::getCredCallbackWrapper);
        }
        else
        {
            oclog() << "Mutex not found";
    ...
    OCStackResult OCSecureResource::provisionCertificate(const char* pemCert,
            ResultCallBack resultCallback)
    {
        if (!resultCallback)
        {
        ...
        if (cLock)
        {
            ...
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCProvisionCertificate(static_cast<void*>(context),
                    devPtr, pemCert,
                    &OCSecureResource::callbackWrapper);
        }
        else
    ...
    OCStackResult OCSecureResource::provisionIdentityCertificate(ResultCallBack resultCallback)
    {
        if (!resultCallback)
        {
            oclog() << "Result calback can't be null";
        ...
        if (cLock)
        {
            ...
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCProvisionIdentityCertificate(static_cast<void*>(context),
                    devPtr, &OCSecureResource::callbackWrapper);
        }
        else
        {
    ...
    OCStackResult OCSecureResource::provisionRoleCertificate(const char* role, const char* authority, ResultCallBack resultCallback)
    {
        if (!resultCallback)
        {
            oclog() << "Result calback can't be null";
        ...
        if (cLock)
        {
            ...
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCProvisionRoleCertificate(static_cast<void*>(context),
                    devPtr, role, authority, &OCSecureResource::callbackWrapper);
        }
        else
        {
            oclog() <<"Mutex not found";
    ...
    OCStackResult OCSecureResource::deleteCred(uint16_t credId,
            ResultCallBack resultCallback)
    {
        if (!resultCallback)
        {
        ...
        if (cLock)
        {
            ...
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCDeleteCredentialByCredId(static_cast<void*>(context),
                    devPtr, credId, &OCSecureResource::callbackWrapper);
        }
        else
        {
            oclog() << "Mutex not found";
    ...
    OCStackResult OCSecureResource::resetDevice(unsigned short waitTimeForOwnedDeviceDiscovery,
            ResultCallBack resultCallback)
    {
        if (!resultCallback)
        {
        ...
        if (cLock)
        {
            ...
            result = OCResetDevice(static_cast<void*>(context), waitTimeForOwnedDeviceDiscovery,
                    devPtr, &OCSecureResource::callbackWrapper);
        }
        else
        {
    ...
    std::string OCSecureResource::getDeviceID()
    {
        ...
        if (!devPtr->doxm | !devPtr->doxm->deviceID.id)
        {
            oclog() << "Invalid secure resource";
            return NULL;
        }
    ...
    uint16_t OCSecureResource::getDevPort()
    {
        validateSecureResource();
        uint16_t port = devPtr->endpoint.port;
        return port;
    }
    ...
    OCStackResult OCSecureResource::getSupportedOTMethods(OicSecOxm_t** oxm, size_t* oxmLen)
    {
        if(!oxm || !oxmLen)
        {
            oclog() << "Null param";
    ...
    OCStackResult OCSecureResource::selectOTMethod( const OicSecOxm_t oxmSelVal,
            ResultCallBack resultCallback)
    {
        if (!resultCallback)
        {
        ...
        if (cLock)
        {
            ...
            std::lock_guard<std::recursive_mutex> lock(*cLock);
            result = OCSelectOTMethod(static_cast<void*>(context),
                    devPtr, oxmSelVal,
                    &OCSecureResource::callbackWrapper);
        }
        else
    ...
    OCStackResult OCSecureResource::setOTMethod( const OicSecOxm_t oxmSelVal)
    {
		devPtr->doxm->oxmSel = oxmSelVal;
        return OC_STACK_OK;
    }
